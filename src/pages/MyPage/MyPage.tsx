...
export default function MyPage() {
  ...
  const [editingTags, setEditingTags] = useState<Array<{tagId: number, genre: string}>>([]);
  ...
  useEffect(() => {
    if (myPageData?.data) {
      ...
      // 중복된 태그 제거
      const uniqueTags = myPageData.data.preferredTags?.filter((tag, index, self) => 
    ...
    <div>
      ...
      <div className="space-y-[14px]">
        ...
        <div className="mt-[24px] px-[16px] relative">
              ...
              {isEditMode && (
                <label className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 cursor-pointer rounded-[100px] opacity-0 hover:opacity-100 transition-opacity">
                  <input
                    ...
                    className="hidden"
                  />
                  <span className="text-white text-[12px] font-medium">이미지 변경</span>
                </label>
              )}
            </div>
          </div>
          ...
          >
            {isEditMode 
              ? (updateProfileMutation.isPending ? "저장 중..." : "프로필 저장하기") 
              : "프로필 수정하기"
            }
          </button>
        </div>
